...
import {QuickOrder} from './modules/quick-order.js';
import {MediaQuery} from './modules/media-query.js';
...
window.Amplify = {
    ...
    /**
     * Register an Amplify module.
     */
    register(name, module) {
        if (this.modules[name]) {
            console.warn(`Amplify module "${name}" is already registered.`);
            return this;
        }

        this.modules[name] = module;

        // Initialize immediately if Amplify is already initialized.
        if (this.initialized) {
    ...
    initializeModule(name, module) {
        ...
        if (typeof module.init !== 'function') {
            console.error(`Amplify module "${name}" is missing an init() function.`);
            return false;
        }

        if (this.hasOwnProperty(name)) {
            console.error(`Amplify has a reserved property name "${name}".`);
            return false;
        }

        try {

            this[name] = module.init(this);
            ...
            console.error(
                `Failed to initialize Amplify module "${name}".`,
                error
            );
    ...
    async loadCartSummary() {
        window.addEventListener(
            'qty-change',
            function (event) {
                let container = document.querySelector('div.x-cart-summary');
    ...
    initPhotoSwipeFromDOM(gallerySelector = '.gallery-wrapper') {
        ...
        var parseThumbnailElements = function (el) {
            ...
            for (var i = 0; i < numNodes; i++) {
                ...
                if ($(linkEl).data('type') === 'video') {
                    ...
                    var size = photoSwipeSizeFromLink(linkEl);
                    item = {
        ...
        var openPhotoSwipe = function (index, galleryElement, disableAnimation, fromURL) {
            ...
            options = {
                closeOnScroll: false,
                // Owl already owns hashes like #item-3; PhotoSwipe history would fight that.
                history: false,
                focus: false,
                // Off-screen Owl slides report bogus thumb bounds and leave zoom leftovers.
    ...
    productSlider: function (target) {
        ...
        const syncProductGalleryThumbs = function (e) {
            ...
            $gallery.find('[data-gallery-index]').removeClass('active');
            $gallery.find('[data-gallery-index="' + index + '"]').addClass('active');
        };

        // Theme scripts may already have initialized this carousel.
        if ($productCarousel.data('owl.carousel')) {
            $productCarousel.on('translate.owl.carousel translated.owl.carousel', syncProductGalleryThumbs);
            return;
        }
        ...
        $productCarousel.owlCarousel({
            ...
            onTranslate: syncProductGalleryThumbs,
        });
    },
