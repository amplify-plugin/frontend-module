...
namespace Amplify\Frontend\Components\Customer\OrderHistory;
    ...
    public function render(): View|Closure|string
    {
        ...
        foreach ($orders as $order) {
            foreach ($order->OrderDetail ?? [] as $item) {
                if (in_array($item->ItemNumber, ['M', 'OT'])) {
                    continue;
                }
                $products[$item->ItemNumber]['items'][] = ['item' => $item, 'order' => $order];
            }
        }
        ...
        return view('widget::customer.order-history.index', [
